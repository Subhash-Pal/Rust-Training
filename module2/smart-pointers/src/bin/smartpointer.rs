// src/bin/smartpointer.rs - CLEANED VERSION (No Warnings)
use std::rc::{Rc, Weak};
...
fn main() {
    ...
    // Use the Cons variant to avoid dead code warning
    let list = List::Cons(1, Box::new(List::Cons(2, 
            Box::new(List::Cons(3, 
                Box::new(List::Nil))))));
    
    // Pattern match to use the values
    if let List::Cons(first_val, _) = &list {
        println!("First value in list: {}", first_val);
    }
    
    println!("Recursive list created successfully");
    
    // Box with large data
    let large_data = Box::new([0u8; 10]);
    println!("Array on heap (first 3): {:?}", &large_data[..3]);
    ...
    println!("Clone 2: {}", rc_clone2);
     ...
     {
      let _rc_clone4 = Rc::clone(&rc_value); 
      println!("Reference count: inside {}", Rc::strong_count(&rc_value));
    }
    ...
    // RC with custom struct (use the fields to avoid warnings)
    #[derive(Debug)]
    struct SharedConfig {
        name: String,
        version: i32,
    ...
    *leaf.parent.borrow_mut() = Rc::downgrade(&branch);
    ...
    // Access and display children to use the field
    ...
    impl TreeNode {
        ...
        fn display_info(&self) {
            ...
            println!("  Children count: {}", self.children.borrow().len());
        }
    }
